...
namespace NotReaper.Tools {
	public class OLD_DragSelect : MonoBehaviour {
		...
		public static OLD_DragSelect Instance = null;
		...
		private void SetHitsoundAction(InternalTargetVelocity velocity) {
		...
		private void CaptureInput() {
			...
			if (primaryModifierHeld && !secondaryModifierHeld && !ModifierHandler.activated && !BookmarkMenu.isActive) {
				...
				if(Input.GetKeyDown(Old_InputManager.selectStandard)) {
					frameIntentSetBehavior = TargetBehavior.Standard;
				}
				else if(Input.GetKeyDown(Old_InputManager.selectChainNode)) {
					frameIntentSetBehavior = TargetBehavior.ChainNode;
				}
				else if(Input.GetKeyDown(Old_InputManager.selectChainStart)) {
					frameIntentSetBehavior = TargetBehavior.ChainStart;
				}
				else if(Input.GetKeyDown(Old_InputManager.selectHold)) {
					frameIntentSetBehavior = TargetBehavior.Sustain;
				}
				else if(Input.GetKeyDown(Old_InputManager.selectHorz)) {
					frameIntentSetBehavior = TargetBehavior.Horizontal;
				}
				else if(Input.GetKeyDown(Old_InputManager.selectMelee)) {
					frameIntentSetBehavior = TargetBehavior.Melee;
				}
				else if(Input.GetKeyDown(Old_InputManager.selectMine)) {
					frameIntentSetBehavior = TargetBehavior.Mine;
				}
				else if(Input.GetKeyDown(Old_InputManager.selectVert)) {
		...
		private void UpdateActions() {
			...
			if (frameIntentSetHitSoundSnare) SetHitsoundAction(InternalTargetVelocity.Snare);
			if (frameIntentSetHitSoundPercussion) SetHitsoundAction(InternalTargetVelocity.Percussion);
			if (frameIntentSetHitSoundChain) SetHitsoundAction(InternalTargetVelocity.Chain);
			if (frameIntentSetHitSoundChainStart) SetHitsoundAction(InternalTargetVelocity.ChainStart);
			if (frameIntentSetHitSoundMelee) SetHitsoundAction(InternalTargetVelocity.Melee);
			if (frameIntentSetHitSoundSilent) SetHitsoundAction(InternalTargetVelocity.Silent);
			...
			if (frameIntentFlipTargetsHorizontally) timeline.FlipSelectedTargetsHorizontal();
			if (frameIntentFlipTargetsVertically) timeline.FlipSelectedTargetsVertical();
			...
			/** Scale notes **/
			if (frameIntentScaleUp) timeline.ScaleSelectedTargets(new Vector2(1.1f, 1.1f));
			if (frameIntentScaleDown) timeline.ScaleSelectedTargets(new Vector2(0.9f, 0.9f));
