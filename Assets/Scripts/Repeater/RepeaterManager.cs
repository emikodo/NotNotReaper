...
namespace NotReaper.Repeaters
{
    public class RepeaterManager : MonoBehaviour
    {
        ...
        public bool AddRepeater(string id, QNT_Timestamp startTime, QNT_Timestamp endTime)
        {
            ...
            var section = new RepeaterSection(id, startTime, endTime, activeEndTime, indicator, targets, timeline);
            indicator.Initialize(miniTimelineParent);
        ...
        public void LoadRepeater(RepeaterSection section)
        {
            ...
            var loadedSection = new RepeaterSection(section.ID, section.startTime, section.activeStartTime, section.endTime, section.activeEndTime, indicator, foundTargets, timeline);
            indicator.Initialize(miniTimelineParent);
        ...
        public List<TargetData> GetMatchingRepeaterTargets(TargetData data)
        {
            ...
            foreach(var segment in repeaters[data.repeaterData.Section.ID])
            {               
                matchingTargets.AddRange(segment.targets.Where(target => target.repeaterData.RelativeTime == data.repeaterData.RelativeTime && target.time != data.time));
            }
